...
import { ChevronLeft, ArrowRight, Github } from 'lucide-react';
        ...
        <div className="bg-[#0B1120] min-h-screen text-[#F1F5F9] relative flex flex-col justify-center items-center px-6 overflow-hidden">
            ...
            >
                <div className="text-center mb-7">
                    ...
                    >
                        {isLogin ? 'Login to ' : 'Join the '}<span className="text-[#D4AF37]">Stack.</span>
                    </motion.h1>
                </div>

                {/* Social Login Button */}
                <button className="w-full bg-[#1E293B] border border-[#F1F5F9]/10 rounded-full py-2.5 flex items-center justify-center gap-3 hover:bg-[#F1F5F9] hover:text-[#0B1120] transition-all duration-300 mb-8 cursor-pointer group">
...
<div className={`grid gap-x-6 gap-y-5 transition-all duration-500 ${!isLogin ? 'grid-cols-2' : 'grid-cols-1'}`}>
    ...
    {!isLogin && (
      <>
        ...
        >
          ...
          <input type="text" placeholder="John Doe" className="w-full text-xs border-b border-[#F1F5F9]/10 bg-transparent pl-2 py-2.5 focus:outline-none focus:border-[#D4AF37] transition-all" />
        </motion.div>
        ...
        initial={{ opacity: 0, scale: 0.8 }}
          ...
          className="space-y-2"
        >
          <label className="text-[10px] uppercase tracking-widest font-bold text-[#F1F5F9]/40">University</label>
          <input type="text" placeholder="UniPort / UNN / Unilag" className="w-full text-xs border-b border-[#F1F5F9]/10 bg-transparent pl-2 py-2.5 focus:outline-none focus:border-[#D4AF37] transition-all" />
        </motion.div>
      </>
    )}
  </AnimatePresence>
  ...
  <div className={`space-y-2 ${!isLogin ? 'col-span-2' : ''}`}>
    ...
    <input type="email" placeholder="john@university.edu" className="w-full text-xs border-b border-[#F1F5F9]/10 bg-transparent pl-2 py-2.5 focus:outline-none focus:border-[#D4AF37] transition-all" />
  </div>

  {/* Password */}
  <div className={`space-y-2 ${!isLogin ? 'col-span-2' : ''}`}>
    <label className="text-[10px] uppercase tracking-widest font-bold text-[#F1F5F9]/40">Password</label>
    <input type="password" placeholder="••••••••" className="w-full text-xs border-b border-[#F1F5F9]/10 bg-transparent pl-2 py-2.5 focus:outline-none focus:border-[#D4AF37] transition-all" />
  </div>
</div>
                    ...
                    <motion.button
                        whileTap={{ scale: 0.98 }}
                        className="w-full bg-[#D4AF37] text-[#0B1120] py-3 rounded-full font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-[#F1F5F9] transition-all shadow-sm mt-8 cursor-pointer"
                    >
